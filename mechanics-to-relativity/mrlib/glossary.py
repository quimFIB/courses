...
The Org file is the source; glossary.js is derived and rebuilt by `mr slides` (and
`mr glossary`). Slides are opened from file://, where a page cannot fetch a sibling
...
ITEM = re.compile(r"^- (.+?) ::(?:[ \t](.*))?$")   # the definition may start on the next line
MATH = re.compile(r"\\\((.+?)\\\)")
...
def parse(org: str) -> list[dict]:
    ...
    for line in org.splitlines():
        ...
        m = ITEM.match(line)
        if m:
            cur = {"term": m.group(1).strip(), "body": (m.group(2) or "").strip(), "section": section}
            entries.append(cur)
...
def build(unit: Path) -> list[str]:
    ...
    js = ("// Generated from GLOSSARY.org by `mr glossary`; edit the Org file, not this one.\n"
