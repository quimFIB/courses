"""Build the whole course as a static site: decks, problem sheets, glossaries, index.

    python3 mrlib/site_build.py [OUT]        # default OUT: _site/   (standard library only)
    ./mr site [--serve]                      # rebuild the glossaries first, then this

What goes in: every unit's slides.html, problems.html and glossary.js, the shared
slides/deck.* and slides/sheet.*, CURRICULUM.html, and a generated index.html. The
problem sheets carry their hints and worked solutions folded under each problem; the
course has no other practice material, so nothing is held back. The copy is an
allow-list, so a new file appears on the site only if this script names its kind.
"""
...
UNIT_FILES = ("slides.html", "problems.html", "glossary.js")
SHARED = ("slides/deck.css", "slides/deck.js", "slides/sheet.css", "slides/sheet.js")
SHARED_GLOBS = ()
...
def unit_info(unit: Path) -> dict:
    ...
    return {
        ...
        "problems": (unit / "problems.html").exists(),
    }
...
def index_html(units: list[dict]) -> str:
    ...
    for part, members in groups.items():
        ...
        for u in members:
            ...
            if u["problems"]:
                links.append(f'<a href="units/{u["dir"]}/problems.html">problems</a>')
...
<title>Mechanics to Relativity</title>
...
<style>
  :root {{
    ...
    --c-root: #151a1d; --c-a: #1f4a54; --c-b: #8c5e12; --c-c: #1f6b3f;
    --c-d: #2a4a80; --c-e: #63397a; --c-f: #8a3a3a;
  ...
  @media (prefers-color-scheme: dark) {{
    :root {{ --ground: #111517; --surface: #1a2023; --ink: #e6eaec; --ink-soft: #a3adb2; --ink-faint: #75808a;
      --rule: #2d3538; --accent: #8cc2cc; --c-root: #e6eaec; --c-a: #8cc2cc; --c-b: #d9a95a;
      --c-c: #7cc59a; --c-d: #9db6e8; --c-e: #c29ad8; --c-f: #e09393; }}
  }}
  * {{ box-sizing: border-box; }}
  ...
  [data-trunk="root"] {{ --trunk: var(--c-root); }} [data-trunk="a"] {{ --trunk: var(--c-a); }}
  [data-trunk="b"] {{ --trunk: var(--c-b); }} [data-trunk="c"] {{ --trunk: var(--c-c); }}
  [data-trunk="d"] {{ --trunk: var(--c-d); }} [data-trunk="e"] {{ --trunk: var(--c-e); }}
  [data-trunk="f"] {{ --trunk: var(--c-f); }}
...
<body>
<main>
  <h1>Mechanics to Relativity</h1>
  <p class="intro">A self-study course from Newton's second law to Einstein's field
  equations, published whole: {len(units)} units, each a slide deck with speaker notes, clickable glossary terms (press <b>G</b>
  in a deck), a reading mode (<b>R</b>) and <b>M</b> to come back here, and a problem sheet with a hint ladder
  and a worked solution folded under every problem. Commands such as <code>./mr problems 07</code> in the decks
  refer to the course repository. See also the <a href="CURRICULUM.html">curriculum</a>, and before unit 00
  the <a href="units/readiness/problems.html">readiness sheet</a>.</p>
{chr(10).join(cards)}
  <footer>{_hub_link()}Decks need a network connection for reveal.js, KaTeX and fonts. Built from the course sources by
  <code>mrlib/site_build.py</code>.</footer>
</main>
</body>
</html>
'''
...
def build(out: Path) -> list[str]:
    ...
    ready = ROOT / "units" / "readiness" / "problems.html"
    if ready.exists():                          # the readiness sheet: no deck, so not a unit card
        dst = out / "units" / "readiness" / "problems.html"
        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ready, dst)
        copied.append("units/readiness/problems.html")
    if (ROOT / "CURRICULUM.html").exists():
        shutil.copy2(ROOT / "CURRICULUM.html", out / "CURRICULUM.html")
        copied.append("CURRICULUM.html")  # the case the 42 problem sheets link to
